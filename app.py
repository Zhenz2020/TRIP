...
def _render_run_inspector_sidebar() -> None:
    st.subheader("\u8fd0\u884c\u68c0\u67e5\u5668")
    # Avoid interrupting an in-flight run: any widget interaction triggers a rerun and aborts the current script.
    if st.session_state.is_running and (time.time() - float(st.session_state.run_heartbeat_ts or 0.0) < 20.0):
...
with _sidebar_inspector_ph.container():
    ...
    for agent_key, label in agent_order:
        ...
        if not opt_names:
            ...
            continue
        choice = st.selectbox(f"{label}", options=opt_names, index=0, key=f"skill_{agent_key}")
    ...
    if runs:
        ...
        sel = st.selectbox("\u9009\u62e9\u4e00\u6b21\u8fd0\u884c", options=options, index=0, key="history_select")
        run_id = sel.split()[1] if sel.split() else ""
        col_a, col_b = st.columns(2)
        with col_a:
            if st.button("\u52a0\u8f7d\u5230\u7ed3\u679c\u9875", key="history_load"):
    ...
    if st.session_state.is_running and (time.time() - float(st.session_state.run_heartbeat_ts or 0.0) < 20.0):
        st.caption("\u6b63\u5728\u8fd0\u884c\u4e2d\uff0c\u6682\u65f6\u9501\u5b9a\u68c0\u67e5\u5668\u4ea4\u4e92\uff0c\u8bf7\u7b49\u5f85\u672c\u6b21\u8fd0\u884c\u5b8c\u6210\u3002")
        if "run_step_outputs" in st.session_state and st.session_state.run_step_outputs:
            st.json(st.session_state.run_step_outputs[-1])
    elif "run_step_outputs" in st.session_state and st.session_state.run_step_outputs:
        steps = st.session_state.run_step_outputs
        opts = [f"{i+1}. {s.get('agent_name','')}" for i, s in enumerate(steps)]
        if "run_step_selected" not in st.session_state:
            st.session_state.run_step_selected = len(steps) - 1
        st.session_state.run_step_selected = st.selectbox(
            "\u9009\u62e9\u6b65\u9aa4",
            options=list(range(len(steps))),
            format_func=lambda i: opts[i],
            ...
            with st.status("\u5904\u7406\u4e2d\u2026", expanded=True) as status:
                ...
                if res is None:
                    ...
                    if assessment is None:
                        assessment = RiskAssessment(risk_level=RiskLevel.UNKNOWN, confidence=0.0, reasons=[], suggested_actions=[])
                    if extract is None:
                    ...
                    res = PipelineResult(
                        ...
                        uploaded_filename=(
                            ", ".join([u.name for u in up_list]) if up_list else None
                        ),
                ...
                # Force one rerun after completion so sidebar inspector can render step outputs.
